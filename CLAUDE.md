...
- `datacollector.py` — Cross-platform data collection via psutil: interface traffic, IP addresses, MAC addresses, system stats (CPU/mem). Platform-specific code handles DHCP leases (dnsmasq + ISC dhcpd formats), neighbor/ARP tables (ip neigh on Linux, arp -an on FreeBSD), default gateway routing, DNS nameservers from resolv.conf, latency via ping, and speedtest results.
